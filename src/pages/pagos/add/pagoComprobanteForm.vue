...
const filteredItems = computed(() => {
  if (isTenant.value || isOwner.value) {
    return items.value.filter(
      item =>
        item.title &&
        (
          item.title.toLowerCase().includes('arriendo') ||
...
const crearPago = async () => {
  ...
  try {
    ...
    setTimeout(() => router.push('/dashboardCliente'), 1500)
  } catch (error) {
  ...
  <div class="add-dano-bg">
    <VCard class="pa-8 add-dano-card mb-6">
      ...
      <VForm @submit.prevent="crearPago">
        <VRow dense>
          <VCol cols="12" md="6">
            <VSelect v-model="pago.item_id" :items="filteredItems" item-title="title" item-value="id" label="Concepto"
              class="mb-4" outlined />
          </VCol>
          <VCol cols="12" md="6">
